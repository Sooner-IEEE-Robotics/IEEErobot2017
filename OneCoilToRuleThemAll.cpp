...
DigitalIn pictureTake(PD_15);

int threshold = 0;

int testing = 0;

int basementCeilingDifference = 0;
...
void calibrate()                 //brates via averaging calNum number of detector reads
{
    ...
    outputToComputer.printf("avg 1 %d \t ", avg);

    avg = avg / aray_size ;
    outputToComputer.printf("%d\n\r",avg);
    ceiling = avg * 1.25;
...
void setup()
{
    ...
    spi.frequency(234375);  //  100000 change back for other people     //setup SPI
...
void start()
{
    ...
    cs = 0;
    spi.write(0x00 | RWBIT);
    spi.write(0xff);
    cs = 1;               //read/write from SPI to test configuration

    cs = 0;
    spi.write(0x00);
    spiTest = spi.write(0x00);
    cs = 1;
    ...
    cs = 0;
    spi.write(ARDUCHIP_FIFO | RWBIT);
    spi.write(FIFO_CLEAR_MASK | FIFO_WRPTR_RST_MASK);
    cs = 1;//resets fifo position, clears fifo memory
    ...
    cs = 0;
    spi.write(ARDUCHIP_FIFO | RWBIT);
    spi.write(FIFO_START_MASK);
    cs = 1;            //tells the camera to begin capturing
...

    {
        ...
        cs = 0;
        spi.write(ARDUCHIP_TRIG);
        data = spi.write(0x00) & CAP_DONE_MASK;
        cs = 1;
...
void writePixelDataToImageArray()
{
    ...
    int fellowshipOfTheCoil = 0;
    int rideOfTheRohirim = 1000;
...
void ultimateMetalDetectionFunction()
{
    ...
    if(metalDetectorReply > (ceiling ))    //controls leds on the nucleo
